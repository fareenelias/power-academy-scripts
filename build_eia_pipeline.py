...
# 2026-09-24: name matching alone let co-ops in ('Union Electric Membership Corp' matched
# 'Union Electric') and missed opcos whose EIA spelling differs ('Louisville Gas & Electric'
# vs 'Louisville Gas and Electric'). Every run now UNIONS the name matches with the verified
# retail-utility IDs from slice_territories_by_id.WHITELIST (ID + NAME checked against the
# HIFLD/EIA-861 file) and DROPS the known false matches below. Name matching stays because
# the generation entities (NEER, Vistra, Talen, XPLR project LLCs) are not in the whitelist.
try:
    from slice_territories_by_id import WHITELIST as _TERR_WHITELIST
    PINNED_IDS = {t: list(v[1]) for t, v in _TERR_WHITELIST.items()}
except Exception as _e:          # never run silently without the pins
    raise SystemExit(f"slice_territories_by_id.WHITELIST not importable ({_e}) - run from the scripts folder")
# Generation affiliates (2026-09-24): nuclear / renewable / merchant fleets that sit in entities
# neither the retail whitelist nor the name matching reaches. Found by a brand scan of the EIA-860
# Operable + Proposed sheets; each id is checked against the expected utility-name fragment below
# (a mismatch is logged loudly and the id is NOT added). Rejected by that scan: East Kentucky Power
# Coop, Old Dominion Electric Coop, TotalEnergies ('talen'), Ohio Power and Light LLC (not AEP).
GEN_AFFILIATE_IDS = {
    "D":    {"5221": "dominion energy nuclear conn", "5248": "dominion energy inc", "58468": "dominion renewable"},
    "ETR":  {"12465": "system energy resources"},
    "NEE":  {"55906": "point beach", "6854": "seabrook", "57026": "montezuma", "57249": "renewable fuels",
             "57460": "wolf ridge", "58289": "desert sunlight 300", "58290": "desert sunlight 250",
             "58353": "majestic", "58515": "mountain view solar", "59115": "seiling", "64421": "nexteraenergy"},
    "TLN":  {"14266": "susquehanna"},
...
def build_territory_geojson_from_xlsx(ticker, match, territories):
    # 2026-09-24: files that carry POLYGONS (slice_territories_by_id.py) are never replaced -
    # this function only writes state-list stubs with features: [], and it used to overwrite
    # every polygon file on each run (the map went blank after the 09-24 run).
    _tp = TERR_DIR / f"{ticker}.geojson"
    if _tp.exists():
        try:
            if json.loads(_tp.read_text(encoding="utf-8")).get("features"):
                log.info(f"{ticker}: territory polygons present - left untouched")
                return
        except Exception:
            pass
    """
...
def match_companies(gen_df: pd.DataFrame, territories: dict) -> dict:
    ...
    for ticker, company in COMPANIES.items():
        ...
        EXCLUDE_PATTERNS = [
            ...
            "town of", "county of", "rural electric", "membership corp",
        ]
        ...
        for gid, frag in GEN_AFFILIATE_IDS.get(ticker, {}).items():
            nm = str(gen_utils.get(gid, ""))
            if gid not in gen_utils:          # proposed-only affiliate (no operable units yet): nothing to check against
                if gid not in matched_ids:
                    matched_ids.append(gid); matched_names.append(f"{frag} (proposed-only)")
                log.info(f"{ticker}: generation affiliate {gid} ({frag}) has no operable units - added on the pinned name")
            elif frag not in nm.lower():
                log.warning(f"{ticker}: generation affiliate {gid} expected '{frag}', EIA has {nm!r} - NOT added")
            elif gid not in matched_ids:
                matched_ids.append(gid); matched_names.append(nm)
        pinned_added = [pid for pid in PINNED_IDS.get(ticker, []) if pid not in matched_ids]
        for pid in pinned_added:
            matched_ids.append(pid); matched_names.append(gen_utils.get(pid, _TERR_WHITELIST[ticker][1][pid]))
        if pinned_added:
            log.info(f"{ticker}: + pinned IDs not found by name: {pinned_added}")
