"""
replace_filings_panel.py — final
Uses a string/comment-aware lexer to find FilingsPanel's true end.
Handles: single/double quoted strings, template literals with ${} nesting,
line comments, block comments.
"""
SRC = r"E:\PowerAcademy\app\poweracademy\src\components\CompanyIntel.js"

def find_fn_end(code, fn_start):
    """Return the index just past the closing } of the function at fn_start."""
    i = fn_start
    # Skip to '(' of parameter list
    while i < len(code) and code[i] != '(':
        i += 1
    # Skip parameter list (paren-balanced)
    paren = 0
    while i < len(code):
        c = code[i]
        if c == '(':   paren += 1
        elif c == ')':
            paren -= 1
            if paren == 0: i += 1; break
        i += 1
    # Find opening { of function body
    while i < len(code) and code[i] != '{':
        i += 1
    # Brace-count with string/comment awareness
    depth = 0
    while i < len(code):
        c = code[i]
        # Line comment
        if c == '/' and i+1 < len(code) and code[i+1] == '/':
            while i < len(code) and code[i] != '\n': i += 1
            continue
        # Block comment
        if c == '/' and i+1 < len(code) and code[i+1] == '*':
            i += 2
            while i < len(code)-1:
                if code[i] == '*' and code[i+1] == '/': i += 2; break
                i += 1
            continue
        # Single / double quoted strings
        if c in ('"', "'"):
            q = c; i += 1
            while i < len(code):
                if code[i] == '\\': i += 2; continue
                if code[i] == q:    i += 1; break
                i += 1
            continue
        # Template literals — track ${} nesting (one level)
        if c == '`':
            i += 1
            tpl = 0
            while i < len(code):
                tc = code[i]
                if tc == '\\':              i += 2; continue
                if tc == '`' and tpl == 0: i += 1; break
                if tc == '$' and i+1 < len(code) and code[i+1] == '{':
                    tpl += 1; i += 2; continue
                if tc == '}' and tpl > 0:   tpl -= 1; i += 1; continue
                i += 1
            continue
        # Regex-like: skip JSX expressions that are just attributes (rare edge)
        if c == '{': depth += 1
        elif c == '}':
            depth -= 1
            if depth == 0: return i + 1
        i += 1
    return -1

with open(SRC, 'r', encoding='utf-8') as f:
    raw = f.read()
code = raw.replace('\r\n', '\n')

fn_start = code.find('\nfunction FilingsPanel(')
if fn_start == -1:
    print("ERROR: FilingsPanel not found"); raise SystemExit(1)

fn_end = find_fn_end(code, fn_start)
if fn_end == -1:
    print("ERROR: could not find end of FilingsPanel"); raise SystemExit(1)

print(f"FilingsPanel: {fn_start} – {fn_end}")
print(f"After end: {repr(code[fn_end:fn_end+80])}")

# Build replacement as list of lines
L = [
'\nfunction FilingsPanel({ ticker }) {',
"  const [filings,       setFilings]       = useState(null);",
"  const [loading,       setLoading]       = useState(true);",
"  const [typeFilter,    setTypeFilter]    = useState('key');",
"  const [searchInput,   setSearchInput]   = useState('');",
"  const [searchQuery,   setSearchQuery]   = useState('');",
"  const [searchResults, setSearchResults] = useState(null);",
"  const [searchLoading, setSearchLoading] = useState(false);",
"  const [cik,           setCik]           = useState(null);",
"  const [snippetMap,    setSnippetMap]    = useState({});",
"  const [snippetLoad,   setSnippetLoad]   = useState({});",
"",
"  const KEY_FORMS   = ['10-K','10-Q','8-K','DEF 14A','S-3','S-4','SC 13D','SC 13G'];",
"  const company     = COMPANIES.find(c => c.ticker === ticker);",
"  const EDGAR_NAMES = { PCG:'PG&E Corp', XIFR:'NextEra Energy Partners', HTO:'SJW Group' };",
"  const entityName  = EDGAR_NAMES[ticker] || company?.name || ticker;",
"  const PROXY       = 'http://100.86.108.51:3001';",
"",
"  const extractCik = (list) => {",
"    for (const f of (list || [])) {",
"      if (f.cik) return String(parseInt(f.cik, 10));",
r"      const m1 = (f.finalLink || '').match(/\/data\/(\d+)\//);",
"      if (m1) return m1[1];",
r"      const m2 = (f.linkToFilingDetails || f.url || '').match(/CIK[=:](\d+)/i);",
"      if (m2) return String(parseInt(m2[1], 10));",
"    }",
"    return null;",
"  };",
"",
"  useEffect(() => {",
"    if (!ticker) return;",
"    setLoading(true);",
"    setSearchResults(null); setSearchQuery(''); setSearchInput('');",
"    setCik(null); setSnippetMap({}); setSnippetLoad({});",
"    const toDate   = new Date().toISOString().split('T')[0];",
"    const fromDate = new Date(Date.now() - 730*24*60*60*1000).toISOString().split('T')[0];",
"    fmpFetch('/stable/sec-filings-search/symbol?symbol=' + ticker + '&from=' + fromDate + '&to=' + toDate + '&limit=100')",
"      .then(f => { const list = Array.isArray(f) ? f : []; setFilings(list); setCik(extractCik(list)); setLoading(false); });",
"  }, [ticker]);",
"",
"  const runSearch = (q) => {",
"    if (!q.trim()) { setSearchResults(null); setSearchQuery(''); return; }",
"    const trimmed = q.trim();",
"    setSearchLoading(true); setSearchQuery(trimmed); setSnippetMap({}); setSnippetLoad({});",
"    const toDate   = new Date().toISOString().split('T')[0];",
"    const fromDate = new Date(Date.now() - 730*24*60*60*1000).toISOString().split('T')[0];",
"    const formsP = (typeFilter !== 'all' && typeFilter !== 'key') ? '&forms=' + encodeURIComponent(typeFilter) : '';",
"    const qs = 'q=' + encodeURIComponent('\"' + trimmed + '\"') +",
"      '&dateRange=custom&startdt=' + fromDate + '&enddt=' + toDate +",
"      '&entity=' + encodeURIComponent(entityName) + formsP;",
"    fetch(PROXY + '/api/edgar-search?' + qs)",
"      .then(r => r.ok ? r.json() : null)",
"      .then(d => { setSearchResults(d && d.hits ? d.hits.hits || [] : []); setSearchLoading(false); })",
"      .catch(() => { setSearchResults([]); setSearchLoading(false); });",
"  };",
"",
"  const clearSearch = () => {",
"    setSearchResults(null); setSearchQuery(''); setSearchInput('');",
"    setSnippetMap({}); setSnippetLoad({});",
"  };",
"",
"  useEffect(() => {",
"    if (!searchResults || !searchResults.length || !searchQuery) return;",
"    setSnippetMap({}); setSnippetLoad({});",
"    searchResults.slice(0, 3).forEach(hit => {",
"      const src = hit._source || {};",
"      const accession = src.accession_no || hit._id || '';",
"      const names = src.display_names || [];",
r"      const cikMatch = names[0] && names[0].match(/CIK (\d+)/);",
"      if (!accession || !cikMatch) return;",
"      const hitCik = parseInt(cikMatch[1], 10);",
"      setSnippetLoad(prev => ({ ...prev, [hit._id]: true }));",
"      fetch(PROXY + '/api/edgar-snippet?accession=' + encodeURIComponent(accession) + '&cik=' + hitCik + '&term=' + encodeURIComponent(searchQuery))",
"        .then(r => r.ok ? r.json() : { snippets: [] })",
"        .then(d => {",
"          setSnippetMap(prev => ({ ...prev, [hit._id]: d.snippets || [] }));",
"          setSnippetLoad(prev => ({ ...prev, [hit._id]: false }));",
"        })",
"        .catch(() => setSnippetLoad(prev => ({ ...prev, [hit._id]: false })));",
"    });",
"  }, [searchResults]);",
"",
"  if (loading) return <Loading />;",
"",
"  const filtered = typeFilter === 'all' ? filings",
"    : typeFilter === 'key' ? (filings||[]).filter(f => KEY_FORMS.includes(f.formType||f.type))",
"    : (filings||[]).filter(f => (f.formType||f.type) === typeFilter);",
"",
"  const inSearch = searchResults !== null;",
"",
"  const filingUrl = (hit) => {",
"    const src = hit._source || {};",
"    const accNo = (src.accession_no || hit._id || '').replace(/-/g, '');",
"    const names = src.display_names || [];",
r"    const cikM = names[0] && names[0].match(/CIK (\d+)/);",
"    if (cikM && accNo) return 'https://www.sec.gov/Archives/edgar/data/' + parseInt(cikM[1],10) + '/' + accNo + '/' + accNo + '-index.htm';",
"    return 'https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=' + (cik||ticker) + '&type=' + (src.form_type||'') + '&dateb=&owner=include&count=40';",
"  };",
"",
"  return (",
"    <div>",
"      <div style={{ display:'flex', gap:6, marginBottom:10, flexWrap:'wrap' }}>",
"        {[{id:'key',label:'Key Filings'},{id:'10-K',label:'10-K'},{id:'10-Q',label:'10-Q'},",
"          {id:'8-K',label:'8-K'},{id:'DEF 14A',label:'Proxy'},{id:'S-3',label:'S-3'},",
"          {id:'S-4',label:'S-4'},{id:'all',label:'All'},",
"        ].map(t => (",
"          <button key={t.id} onClick={() => setTypeFilter(t.id)}",
"            style={{ padding:'4px 10px', borderRadius:20, fontSize:11, cursor:'pointer',",
"              border:'1px solid ' + (typeFilter===t.id ? COLORS.accent : COLORS.border),",
"              background:typeFilter===t.id ? COLORS.accentLo : 'transparent',",
"              color:typeFilter===t.id ? COLORS.accent : COLORS.textMid,",
"              fontWeight:typeFilter===t.id ? 700 : 400 }}>",
"            {t.label}",
"          </button>",
"        ))}",
"      </div>",
"",
"      <div style={{ display:'flex', gap:8, marginBottom:14, alignItems:'center' }}>",
"        <div style={{ flex:1, position:'relative' }}>",
"          <span style={{ position:'absolute', left:10, top:'50%', transform:'translateY(-50%)', color:COLORS.textLo, fontSize:12, pointerEvents:'none' }}>&#128269;</span>",
"          <input value={searchInput}",
"            onChange={e => setSearchInput(e.target.value)}",
"            onKeyDown={e => e.key === 'Enter' && runSearch(searchInput)}",
"            placeholder={'Search inside ' + entityName + ' SEC filings...'}",
"            style={{ width:'100%', background:COLORS.bg, borderRadius:6, fontSize:12,",
"              color:COLORS.text, outline:'none', boxSizing:'border-box', fontFamily:'inherit',",
"              padding:'7px 10px 7px 32px',",
"              border:'1px solid ' + (inSearch ? COLORS.accent : COLORS.border) }}",
"          />",
"        </div>",
"        <button onClick={() => runSearch(searchInput)}",
"          style={{ padding:'7px 14px', borderRadius:6, background:COLORS.accent,",
"            color:'#fff', border:'none', fontSize:12, fontWeight:600, cursor:'pointer', whiteSpace:'nowrap' }}>",
"          Search",
"        </button>",
"        {inSearch && (",
"          <button onClick={clearSearch}",
"            style={{ padding:'7px 12px', borderRadius:6, background:'transparent',",
"              color:COLORS.textMid, border:'1px solid ' + COLORS.border, fontSize:12, cursor:'pointer' }}>",
"            Clear",
"          </button>",
"        )}",
"      </div>",
"",
"      {inSearch ? (",
"        searchLoading ? <Loading /> : (",
"          <div>",
"            <div style={{ fontSize:11, color:COLORS.textLo, marginBottom:10 }}>",
"              {searchResults.length > 0",
"                ? searchResults.length + ' filing' + (searchResults.length !== 1 ? 's' : '') + ' contain \\u201c' + searchQuery + '\\u201d'",
"                : 'No filings found containing \\u201c' + searchQuery + '\\u201d'}",
"              <span style={{ margin:'0 8px', color:COLORS.border }}>&middot;</span>",
"              <span>SEC EDGAR full-text search</span>",
"            </div>",
"            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>",
"              {searchResults.map((hit, i) => {",
"                const src = hit._source || {};",
"                const hitSnips = snippetMap[hit._id] || [];",
"                const isLoading = snippetLoad[hit._id];",
"                return (",
"                  <div key={i} style={{ background:COLORS.bg, border:'1px solid ' + COLORS.border, borderRadius:8, padding:'12px 14px' }}>",
"                    <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8 }}>",
"                      <Pill label={src.form_type||'--'} color={COLORS.accent} />",
"                      <span style={{ fontSize:11, color:COLORS.textLo, whiteSpace:'nowrap' }}>{src.file_date||'--'}</span>",
"                      {src.period_of_report && <span style={{ fontSize:10, color:COLORS.textLo }}>period {src.period_of_report}</span>}",
"                      <span style={{ flex:1 }} />",
"                      <a href={filingUrl(hit)} target='_blank' rel='noopener noreferrer'",
"                        style={{ fontSize:11, fontWeight:600, color:COLORS.accent, textDecoration:'none', whiteSpace:'nowrap' }}>",
"                        View on EDGAR",
"                      </a>",
"                    </div>",
"                    <div style={{ borderTop:'1px solid ' + COLORS.border + '33', paddingTop:8 }}>",
"                      {isLoading ? (",
"                        <div style={{ fontSize:11, color:COLORS.textLo, fontStyle:'italic' }}>Loading preview...</div>",
"                      ) : hitSnips.length > 0 ? (",
"                        <div style={{ display:'flex', flexDirection:'column', gap:5 }}>",
"                          {hitSnips.map((s, j) => {",
"                            const html = '\\u2026' + s.split('[[').join('<mark style=\"background:#2a2000;color:#f4c542;padding:0 2px;border-radius:2px\">').split(']]').join('</mark>') + '\\u2026';",
"                            return <div key={j} style={{ fontSize:11, color:COLORS.textMid, lineHeight:1.7, background:COLORS.panel, borderRadius:4, padding:'5px 8px', borderLeft:'2px solid ' + COLORS.accent + '40' }} dangerouslySetInnerHTML={{ __html: html }} />;",
"                          })}",
"                        </div>",
"                      ) : i < 3 ? (",
"                        <div style={{ fontSize:11, color:COLORS.textLo, fontStyle:'italic' }}>Term not found in first 500KB of filing</div>",
"                      ) : (",
"                        <div style={{ fontSize:11, color:COLORS.textLo, fontStyle:'italic' }}>Click View to open on EDGAR</div>",
"                      )}",
"                    </div>",
"                  </div>",
"                );",
"              })}",
"            </div>",
"          </div>",
"        )",
"      ) : (",
"        <div style={{ background:COLORS.bg, border:'1px solid ' + COLORS.border, borderRadius:8, overflow:'hidden' }}>",
"          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>",
"            <thead>",
"              <tr style={{ borderBottom:'1px solid ' + COLORS.border }}>",
"                {['Type','Date','Description','Link'].map(h => (",
"                  <th key={h} style={{ padding:'10px 12px', textAlign:'left', color:COLORS.textLo, fontWeight:600, fontSize:11, textTransform:'uppercase', letterSpacing:'0.06em' }}>{h}</th>",
"                ))}",
"              </tr>",
"            </thead>",
"            <tbody>",
"              {(filtered||[]).slice(0,20).map((f,i) => (",
"                <tr key={i} style={{ borderBottom:'1px solid ' + COLORS.border, background:i%2===0?'transparent':COLORS.panel }}>",
"                  <td style={{ padding:'9px 12px' }}><Pill label={f.formType||f.type||'--'} color={COLORS.accent} /></td>",
"                  <td style={{ padding:'9px 12px', color:COLORS.textLo, whiteSpace:'nowrap' }}>{(f.filedAt||f.fillingDate||'--').split('T')[0].split(' ')[0]}</td>",
"                  <td style={{ padding:'9px 12px', color:COLORS.textMid, maxWidth:300, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{f.description||f.formType||f.type||'--'}</td>",
"                  <td style={{ padding:'9px 12px' }}>",
"                    {(f.finalLink||f.linkToFilingDetails||f.url) && <a href={f.finalLink||f.linkToFilingDetails||f.url} target='_blank' rel='noopener noreferrer' style={{ color:COLORS.accent, textDecoration:'none', fontSize:11, fontWeight:600 }}>View</a>}",
"                  </td>",
"                </tr>",
"              ))}",
"            </tbody>",
"          </table>",
"          {(!filtered||filtered.length===0) && <Empty msg='No filings found' />}",
"        </div>",
"      )}",
"    </div>",
"  );",
"}",
]

PANEL = '\n'.join(L)
code = code[:fn_start] + PANEL + code[fn_end:]

with open(SRC, 'w', encoding='utf-8') as f:
    f.write(code)
print("Done.")