...
Usage:
  ...
  python3 scripts/build_guidance_history.py --refresh-index --tickers AEP  # after build_corpus.py: re-project guidance_index.json, then build
"""
...
MANIFEST = os.path.join(BASE, 'data', 'corpus_manifest.json')


def refresh_index():
    """guidance_index.json is a projection of corpus_manifest.json: every earnings-call
    presentation with a period and a date (2026-09-23: verified 419/419 ids identical to the
    hand-built 08-06 file, then AEP's 16 decks were the only additions). Re-run after
    build_corpus.py whenever a new name or quarter lands; the row shape is what build_row reads."""
    docs = json.load(open(MANIFEST, encoding='utf-8'))['documents']
    # A JOINT deck belongs to every coverage name on it (2026-09-24): the 2021-24 NEE/NEP decks
    # are tickered ['XIFR', 'NEE'] and were indexed under XIFR alone, so NEE lost 13 vintages.
    # The first ticker is always indexed (unchanged); a further ticker only if it is already a
    # guidance-layer name, so a peer mentioned in a title can never create a new ticker here.
    try:
        known = {r['ticker'] for r in json.load(open(INDEX, encoding='utf-8'))}
    except Exception:
        known = set()
    rows = []
    for d in docs:
        if d.get('doc_type') != 'presentation' or d.get('event') != 'earnings_call':
            continue
        if not d.get('period') or not d.get('date') or not d.get('tickers'):
            continue
        for tk in [d['tickers'][0]] + [t for t in d['tickers'][1:] if t in known]:
            rows.append({'id': d['id'], 'ticker': tk, 'period': d['period'], 'date': d['date'],
                         'title': d['title'], 'pages': d['pages'], 'words': d['words'], 'url': d['url'],
                         'text': d['text_file']})
    # One deck per ticker-period (build_row would abort on a collision). When the same call has
    # several presentation PDFs (AEP files its financial-supplement tables as a second 'Earnings
    # Call' deck) keep the longest, which is the slide deck, and say which one was set aside.
    by_tp = collections.defaultdict(list)
    for r in rows:
        by_tp[(r['ticker'], norm_period(r['period']))].append(r)
    kept = []
    for (tk, pk), grp in sorted(by_tp.items()):
        grp.sort(key=lambda r: (-(r['pages'] or 0), -(r['words'] or 0), r['id']))
        kept.append(grp[0])
        for r in grp[1:]:
            print('  %s %s: set aside %s (%s pp) in favour of %s (%s pp)' %
                  (tk, pk, r['url'].rsplit('/', 1)[-1], r['pages'], grp[0]['url'].rsplit('/', 1)[-1], grp[0]['pages']))
    rows = kept
    rows.sort(key=lambda r: (r['ticker'], r['date'], r['id']))
    old = json.load(open(INDEX, encoding='utf-8')) if os.path.exists(INDEX) else []
    old_ids = {r['id'] for r in old}
    new_ids = {r['id'] for r in rows} | {r['id'] for g in by_tp.values() for r in g}
    lost = old_ids - new_ids
    if lost:
        sys.exit('ABORT: refreshed guidance_index would drop %d existing deck(s) - is corpus_manifest.json complete?' % len(lost))
    fd, tmp = tempfile.mkstemp(dir=os.path.dirname(INDEX), suffix='.tmp')
    with os.fdopen(fd, 'w', encoding='utf-8') as fh:
        json.dump(rows, fh, indent=1)
    os.replace(tmp, INDEX)
    old_tid = {(r['ticker'], r['id']) for r in old}   # by (ticker, deck): a joint deck gained by a 2nd name counts
    gained = collections.Counter(r['ticker'] for r in rows if (r['ticker'], r['id']) not in old_tid)
    print('guidance_index.json: %d decks (+%d: %s)' % (len(rows), sum(gained.values()),
          ', '.join('%s %d' % kv for kv in sorted(gained.items())) or 'none'))
...
CAP_RECIPES = {
 'AEP': [   # 2026-09-23: surveyed all 16 decks Q4 2021-Q2 2026; Q4 2022 (Feb-2023) prints no plan total
   (W + r'\s+Capital Forecast of\s+' + AMT + r'\s*B', hw('v')),                      # 2024-2028 Capital Forecast of $43B
   (AMT + r'\s*B\s+(?:Updated\s+)?Capital Plan\s+' + W, hw('v')),                    # $78B Updated Capital Plan 2026-2030
   (AMT + r'\s*B\s+' + W + r'\s+Capital Forecast', hw('v')),                          # $54B 2025-2029 Capital Forecast
   (AMT + r'\s*B\s+capital plan from\s+' + W, hw('v')),                              # $43B capital plan from 2024-2028
   (AMT + r'\s*B\s+capital plan for the\s+' + W + r'\s+forecast period', hw('v')),
   (r'(?:5-year|five-year) capital plan of\s+' + AMT + r'\s*B', lambda m: {'value': float(m.group('v')),
        'window': None, 'basis': 'printed'}),                                          # 5-year capital plan of $54B
   (AMT + r'\s*B,?\s+(?:updated\s+|base\s+|conservative\s+)?(?:5-year\s+|five-year\s+)?capital plan\b',
      lambda m: {'value': float(m.group('v')), 'window': None, 'basis': 'printed'}),   # $72B 5-year capital plan
   (r'capital plan of\s+' + AMT + r'\s*B', lambda m: {'value': float(m.group('v')), 'window': None, 'basis': 'printed'}),
 ],
 'AEE': [
...
# REFERENCE-RANGE GUARD (2026-09-24). EIX prints, on the same key-messages slide as
# its live guidance, the range a CAGR is MEASURED FROM ("based on the midpoint of the
# initial 2021 EPS guidance range of $4.42-4.62") or a range DERIVED from a target
# ("would result in 2025 EPS of $5.50-5.90"). Equal score + same slide -> the tie
# guard refused, and 19 of 21 EIX vintages stored nothing. A range introduced this
# way is a reference, not this vintage's guidance: pushed down hard.
# A bare 'midpoint of 2022 EPS guidance range of' is NOT a reference - AEE's footnotes
# describe the live range that way - so 'original' / 'initial' is required (an
# out-year range introduced as a midpoint is caught by the year binding instead).
_EPS_REF = re.compile(r'(?i)(?:(?:original|initial) (?:20\d\d )?(?:core |adjusted )?EPS guidance range'
                      r'|based (?:on )?(?:the )?midpoint of (?:the )?(?:20\d\d )?(?:core |adjusted )?EPS guidance range'
                      r'|would result in (?:20\d\d )?(?:core |adjusted )?EPS of'
                      r'|starting point of)[^$]{0,25}$')
def _eps_own_year(fb, s, e):
    """The year a range is GRAMMATICALLY bound to, or None: '$6.74-7.14 (2028)' or
    '2027 Core EPS guidance of $6.25-6.65'. Deliberately narrow - the first cut read
    the nearest year on the slide and that year is usually layout noise ('$4.39 -
    $4.49 2020 EPS of $3.84', '$1.75 to $1.80 2021 - 2024', a column header two
    cells away); the regression scan caught it wiping 10 correct AWK/PCG/WTRG cells."""
    m = re.match(r'\s*\((20[2-3]\d)\)', fb[e:e + 9])
    if m: return int(m.group(1))
    # trailing label: 'Introduced $4.75-5.05 2024 Core EPS Guidance' (EIX key-messages tiles)
    m = re.match(r'\s*(20[2-3]\d)\s+(?:core |adjusted |operating |diluted )?(?:EPS|earnings per share)\d?\s+guidance',
                 fb[e:e + 50], re.I)
    if m: return int(m.group(1))
    m = re.search(r'\b(20[2-3]\d)\s+(?:core |adjusted |operating |diluted )?(?:EPS|earnings per share)\d?\s+'
                  r'guidance(?:\s+(?:of|to|at|range(?:\s+(?:of|for))?))?\s*$', fb[max(0, s - 70):s], re.I)
    return int(m.group(1)) if m else None

# YEAR-HEADER TABLE FALLBACK (2026-09-24). NEE prints guidance ONLY as an
# 'Adjusted EPS Expectations' table - '2024 2025E 2026E 2027E ... $3.45-$3.70
# $3.63-$4.00 $3.85-$4.32' - with no 'guidance' word, so the scorer dropped every
# candidate and NEE had zero EPS cells. The text order of the cells is NOT reliable
# (Q3 2021 extracts 2023E first), so ranges are ordered by value: an expectations
# table rises year on year, and the guard below refuses any table where that is not
# strictly true. Only the FIRST estimate column is stored (the vintage's guidance
# year); it must be fy or fy+1. Used only when the scorer found nothing at all.
_EPS_TABLE_PAGE = re.compile(r'(?i)(?:EPS|earnings per share|financial) expectations')
_EPS_TABLE_HDR = re.compile(r'\b20[2-3]\d\s+((?:20[2-3]\dE\s*){1,6})')
def eps_table(fb, fy):
    if not _EPS_TABLE_PAGE.search(fb): return None
    h = _EPS_TABLE_HDR.search(fb)
    if not h: return None
    eyrs = [int(y) for y in re.findall(r'(20[2-3]\d)E', h.group(1))]
    if not eyrs or eyrs[0] not in (fy, fy + 1): return None
    rngs = []
    for m in EPS_RANGE.finditer(fb, h.end()):
        if re.search(r'(?i)range of\s*$', fb[max(0, m.start() - 20):m.start()]): continue  # footnote ('2024 adjusted EPS range of ...')
        lo, hi = float(m.group('lo')), float(m.group('hi'))
        if not (0.2 <= lo < hi <= 30) or (hi - lo) > 0.35 * lo: continue
        if (lo, hi) not in rngs: rngs.append((lo, hi))
    if not rngs or len(rngs) > len(eyrs): return None
    rngs.sort()
    if any(not (b[0] > a[0] and b[1] > a[1]) for a, b in zip(rngs, rngs[1:])): return None
    lo, hi = rngs[0]
    return {'year': str(eyrs[0]), 'low': lo, 'high': hi, 'midpoint': round((lo + hi) / 2.0, 4),
            'basis': 'printed', 'table': list(zip(eyrs, rngs)),
            'text': fb[max(0, h.start() - 130): h.end() + 160]}

# Tickers that do not guide EPS at all. XIFR (NextEra Energy Partners, an LP) guides
# distributions per unit; its 2021-24 decks are JOINT NEE/NEP decks that carry NEE's
# 'Adjusted EPS Expectations' table - the table fallback would store NEE's EPS as
# XIFR's (14 cells, caught by the 2026-09-24 regression scan).
NO_EPS_GUIDANCE = {'XIFR': "guides distributions per unit, not EPS; joint NEE/NEP decks carry NEE's EPS table"}
...
def eps_guidance(tk, deck):  # noqa: C901
    ...
    exactly how a wrong-but-plausible EPS cell gets created. Reference ranges
    (a CAGR base, a derived out-year figure) and ranges labelled with a year other
    than fy / fy+1 are pushed down too (2026-09-24). A year-header expectations
    table is the fallback when nothing scores (2026-09-24)."""
    if tk in NO_EPS_GUIDANCE:
        return None
    fy = int(deck['period'].split()[-1])
    cands = []
    for pno, body in pages(deck['id']):
        ...
        for m in EPS_RANGE.finditer(fb):
            ...
            if _EPS_REF.search(pre[-120:]): sc -= 6
            own = _eps_own_year(fb, m.start(), m.end())
            if own is not None and own > fy + 1: sc -= 6   # an out-year target (fy+2 on), not this vintage's guidance
            if sc <= 2:
                continue
            if own in (fy, fy + 1):
                yr = str(own)
            else:
                # NEAREST fy/fy+1 year to the range, not the last one in the window (2026-09-24):
                # 'EPS 2023 $1.19-$1.23 Up at least 10% $0 2024 Up at least 10%' is 2023's range
                # (PCG Q1-Q3 2023 were labelled 2024); '$1.33 - $1.37 2024 Guidance ... 2025' is 2024's.
                win_ = pre[-90:] + ' ' + post[:60]; at = len(pre[-90:])
                yrs = [(min(abs(ym.start() - at), abs(ym.end() - at)), ym.group(1))
                       for ym in re.finditer(r'\b(20[2-3]\d)\b', win_) if int(ym.group(1)) in (fy, fy + 1)]
                # ...except a Q4 deck, whose guidance range is next year's by convention: fy+1 wins
                # when it is in the window ('$2.90 - $3.00 2024 Adjusted EPS 2025 Guidance', HTO Q4 2024).
                if deck['period'].split()[0] in ('Q4', 'FY') and any(int(y) == fy + 1 for _, y in yrs):
                    yr = str(fy + 1)
                else:
                    yr = min(yrs)[1] if yrs else None
            cands.append(((sc, -pno), {'year': yr, 'low': lo, 'high': hi,
                                       'midpoint': round((lo + hi) / 2.0, 4), 'basis': 'printed', 'page': pno,
                                       'text': fb[max(0, m.start() - 130): m.end() + 90]}))
    if not cands:
        for pno, body in pages(deck['id']):
            t = eps_table(flat(body), fy)
            if t:
                t['page'] = pno
                return t
        return None
...
# Embedded EPS controls (2026-09-24) - REAL slide texts. Runs at import; a failure
# refuses the build. Each is (period, text, expected (year, lo, hi) or None).
_EPS_CONTROLS = [
    # EIX Q3 2021: live narrowed range beside the CAGR-base reference range
    ('Q3 2021', "Compound annual growth rate (CAGR) based on the midpoint of the initial 2021 EPS guidance range of $4.42–4.62 November 2, 2021 EIX narrows 2021 core EPS guidance to $4.42–4.52", ('2021', 4.42, 4.52)),
    # EIX Q4 2024: revised guidance + CAGR base + a (2028) out-year figure on one slide
    ('Q4 2024', "Revised $5.94–6.34 2025 Core EPS Guidance1 2. Compound annual growth rate (CAGR) based on the midpoint of the original 2025 EPS guidance range of $5.50–5.90 plus run-rate interest expense reduction of 14¢ Continued confidence in delivering 5–7% Core EPS1 growth from $5.84 (2025) to $6.74–7.14 (2028)", ('2025', 5.94, 6.34)),
    # EIX Q4 2025: next-year guidance beside the year-after (fy+2) guidance
    ('Q4 2025', "Introduced 2026 Core EPS1 guidance of $5.90–6.20 and 2027 Core EPS guidance of $6.25–6.65 1. See Earnings Per Share", ('2026', 5.90, 6.20)),
    # EIX Q2 2023: derived 2025 figure must not win over live 2023 guidance
    ('Q2 2023', "EIX reaffirms 2023 core EPS guidance of $4.55–4.85 EIX reaffirms 5–7% 2021–2025 EPS growth rate target, which would result in 2025 EPS of $5.50–5.90", ('2023', 4.55, 4.85)),
    # NEE Q4 2024 table: footnote range skipped, first estimate column stored
    ('Q4 2024', "Adjusted Earnings Per Share Expectations 2. 2024 adjusted EPS range of $3.23 - $3.43 2024 2025E 2026E 2027E $3.43 6% - 8% off 2024 adjusted EPS range2 $3.45 – $3.70 $3.63 – $4.00 $3.85 – $4.32", ('2025', 3.45, 3.70)),
    # NEE Q3 2021 table: cells extract out of order - value order decides
    ('Q3 2021', "NextEra Energy’s Adjusted Earnings Per Share Expectations 2020 2021E 2022E 2023E • Expect adjusted EPS growth in the range of 6% to 8% $2.77 - $2.97 We will be disappointed $2.55 - $2.75 $2.40 - $2.54 $2.31", ('2021', 2.40, 2.54)),
    # NEE Q4 2025 table: one range, first estimate column
    ('Q4 2025', "NextEra Energy’s Financial Expectations1 2025 2026E 2030E 2032E 2035E $3.92–$4.02 $3.71 8%+ Long-term Expectations2", ('2026', 3.92, 4.02)),
    # table guard: nested ranges (not a year-on-year rising table) are refused
    ('Q4 2024', "Financial Expectations 2024 2025E 2026E $3.40 – $4.20 $3.60 – $3.90", None),
    # table guard: header whose first estimate year is not the guidance year
    ('Q4 2024', "Financial Expectations 2027 2028E 2029E $3.45 – $3.70 $3.63 – $4.00", None),
    # AEE Q4 2021: a footnote naming the LIVE range's midpoint is not a reference
    ('Q4 2021', "Represents midpoint of 2022 EPS guidance range of $3.95 to $4.15. 3 Unrounded dividends", ('2022', 3.95, 4.15)),
    # EIX Q3 2023: an out-year range introduced as a midpoint loses to the live guidance
    ('Q3 2023', "EIX reaffirms 2023 core EPS guidance of $4.55\u20134.85 1. For 2025, represents the midpoint of the 2025 core EPS guidance range for $5.50\u20135.90 $5.70", ('2023', 4.55, 4.85)),
    # EIX Q1 2024: a fy+1 range that is only a CAGR base does not tie with the live guidance
    ('Q1 2024', "Affirmed $4.75\u20135.05 2024 Core EPS Guidance1 3. Compound annual growth rate (CAGR) based on the midpoint of the 2025 EPS guidance range of $5.50\u20135.90 2 4", ('2024', 4.75, 5.05)),
    # EIX Q4 2023: the guidance year is the trailing tile label, not the deck year
    ('Q4 2023', "$4.76 2023 Core EPS1 Reiterated 5\u20137% Core EPS CAGR 2025\u201320283 Introduced $4.75\u20135.05 2024 Core EPS Guidance1 2023 Core EPS above midpoint", ('2024', 4.75, 5.05)),
    # table guard: more ranges than estimate columns (a footnote/segment range leaked in) is refused
    ('Q4 2024', "Financial Expectations 2024 2025E 2026E $3.45 \u2013 $3.70 $3.63 \u2013 $4.00 $3.85 \u2013 $4.32", None),
    # PCG Q1 2023: the year label nearest the range wins, not the last year in the window
    ('Q1 2023', "Reaffirming 2023 Guidance EPS New Equity 2023 $1.19-$1.23 Up at least 10% $0 2024 Up at least 10% $0", ('2023', 1.19, 1.23)),
    # HTO Q4 2024: in a Q4 deck fy+1 wins over an adjacent prior-year label
    ('Q4 2024', "anchored off 2022 reported EPS of $2.43 $2.90 - $3.00 2024 Adjusted EPS 2025 Guidance Adjusted", ('2025', 2.90, 3.00)),
    # unchanged behaviour: a plain headline guidance range still extracts
    ('Q2 2021', "Re-affirming full year 2021 EPS guidance of $2.70 to $2.85 per diluted share", ('2021', 2.70, 2.85)),
]

def _run_eps_controls():
    for i, (per, txt, want) in enumerate(_EPS_CONTROLS):
        pid = '__eps_ctl_%d' % i
        _pc[pid] = [(1, txt)]
        try:
            got = eps_guidance('CTL', {'id': pid, 'period': per})
        finally:
            _pc.pop(pid, None)
        trip = None if (not got or got.get('ambiguous')) else (got['year'], got['low'], got['high'])
        assert trip == want, 'EPS CONTROL %d FAILED: %r -> %r, want %r' % (i, txt[:60], trip, want)
    # XIFR gate: NEE's own table (control 4 text) must NOT extract under XIFR
    _pc['__eps_ctl_x'] = [(1, _EPS_CONTROLS[4][1])]
    try:
        got = eps_guidance('XIFR', {'id': '__eps_ctl_x', 'period': 'Q4 2024'})
    finally:
        _pc.pop('__eps_ctl_x', None)
    assert got is None, 'EPS CONTROL XIFR FAILED: NEE table extracted under XIFR -> %r' % (got,)
...
_LT_MID = re.compile(r'shareholder\s+return|\bTSR\b', re.I)   # a TSR label between the range and the EPS word

def lt_growth_scan(fb, passes=(1, 2)):
    """First match only (the controls' contract). See _lt_iter for every match."""
    for r in _lt_iter(fb, passes):
        return r
    return None

def _lt_iter(fb, passes=(1, 2)):
    """Per-text LT-EPS-growth scan; module-level so the controls can run on strings.
    pass 1 = range BEFORE the EPS word ('6% to 8% EPS CAGR 2026-2030'); pass 2 = EPS word
    first ('EPS growth ... near upper end of 6% to 8%'), the looser form."""
    rx = (r'(?P<lo>\d{1,2}(?:\.\d)?)\s*%\s*(?:' + DASH + r'|to|-)\s*(?P<hi>\d{1,2}(?:\.\d)?)\s*%[^.%]{0,70}?'
          r'(?:EPS|earnings per share|adjusted EPS|operating earnings)[^.%]{0,30}?(?:CAGR|growth)')
    rx2 = (r'(?:EPS|earnings per share|operating earnings)[^.%]{0,40}?(?:CAGR|growth)[^.%]{0,40}?(?P<lo>\d{1,2}(?:\.\d)?)\s*%'
           r'\s*(?:' + DASH + r'|to|-)\s*(?P<hi>\d{1,2}(?:\.\d)?)\s*%')
    # pass 3 (2026-09-24): NEE's '6 to 8% annual growth rate through 2026, off the 2024 adjusted
    # EPS expectations range' - no '%' on the low end, and 'growth' comes BEFORE 'EPS'. Anchored
    # on 'annual growth rate' + 'off the YYYY adjusted EPS' so a load/dividend growth rate can't fit.
    rx3 = (r'(?P<lo>\d{1,2}(?:\.\d)?)\s*%?\s*(?:' + DASH + r'|to|-)\s*(?P<hi>\d{1,2}(?:\.\d)?)\s*%\s*annual growth rate'
           r'[^.%]{0,40}?off (?:the )?20\d\d (?:adjusted |operating )?(?:EPS|earnings per share)')
    for r_ in [x for i, x in ((1, rx), (2, rx2), (3, rx3)) if i in passes]:
        for m in re.finditer(r_, fb, re.I):
            ...
            # a bullet starts a new clause (2026-09-24): '...quarterly common stock dividend to $0.2575 per
            # share \u27a2Extended 6% - 8% annual EPS ... growth' (PPL Q4 2023) - the dividend belongs to the
            # previous bullet, not to our range.
            bcut = max(pre.rfind(ch) for ch in '\u27a2\u2022\u25aa\u25cf\uf0a7')
            if bcut >= 0: pre = pre[bcut + 1:]
            # '... Rate Base CAGR through 2030 7%-9% Operating Earnings Growth': a clause that
            # closes on a through-year belongs to the figure BEFORE it, not to ours (AEP 2026-09-23).
            if re.search(r'\b(?:through|to|by)\s+(?:at least\s+)?20\d\d\W*$', pre, re.I): pre = ''
            if _LT_COLLIDE.search(pre): continue
            # The range must not be labelled by a collide word BETWEEN it and the EPS word
            # either ('~10%-12% Total Shareholder Return ... EPS growth of 6%-8%', AEP Q2 2025).
            if _LT_MID.search(fb[m.end('hi'): m.end()]): continue
            seg = fb[max(0, m.start() - 140): m.end() + 160]
            # 'vs' removed 2026-09-24: in grid slides it binds to a neighbouring metric's comparison year
            # ('~$345M or >25% reduction 2025E vs 2018A' - EVRG O&M) and gave an EPS algorithm a 2018 base.
            by = re.search(r'(?:off|from|using|base(?:d)? (?:off|on))[^.]{0,60}?(20\d\d)', seg)
            ...
            if win_m and int(win_m.group(1)) < int(win_m.group(2)):
                ...
                _wy = None
            yield {'rate_low_pct': lo, 'rate_high_pct': hi,
                   'base_year': _wy[0] if _wy else (by.group(1) if by else None),
                   'through_year': _wy[1] if _wy else (thru.group(1) if thru else None),
                   'basis': 'printed',
                   'text': fb[max(0, m.start() - 120): m.end() + 120]}

def lt_growth(tk, deck):
    """VALUE-PRESERVING page choice (2026-09-24). The RATE is what it always was: the first match
    in page order (passes 1+2 per page). Among every match in the deck with that SAME rate, the one
    carrying the most window metadata (base year, through year) supplies the page and the years;
    ties keep the first. Fixes AEE ('EPS growth ... 6% to 8%' on p11 won over '6% to 8% EPS CAGR
    2026-2030' on p17) without the POR/D base-year loss the 09-24 pass-order attempt caused.
    Pass 3 is tried only when passes 1-2 find nothing anywhere in the deck."""
    if tk in NO_EPS_GUIDANCE:      # XIFR: joint NEE/NEP decks carry NEE's EPS algorithm (2026-09-24)
        return None
    pid = deck['id']
    allm = []
    for pno, body in pages(pid):
        for r in _lt_iter(flat(body), (1, 2)):
            r['page'] = pno
            allm.append(r)
    if not allm:
        for pno, body in pages(pid):
            for r in _lt_iter(flat(body), (3,)):
                r['page'] = pno
                allm.append(r)
    if not allm:
        return None
    first = allm[0]
    same = [r for r in allm if (r['rate_low_pct'], r['rate_high_pct']) == (first['rate_low_pct'], first['rate_high_pct'])]
    meta = lambda r: (r['base_year'] is not None) + (r['through_year'] is not None)
    best = max(same, key=meta)          # max() keeps the FIRST among equals
    return best if meta(best) > meta(first) else first
...
_LT_CONTROLS = [
    ...
    # PPL Q4 2023 (2026-09-24): a 'dividend' in the PREVIOUS bullet must not refuse the range
    ("Announces 7.3% increase to quarterly common stock dividend to $0.2575 per share \u27a2Extended 6% - 8% annual EPS and dividend growth targets through at least 2027", (6.0, 8.0)),
    # ...but a dividend in the SAME clause still refuses
    ("\u2022 dividend growth of 6% - 8% annual EPS growth", None),
    ...
    # AEP 2026-09-23: TSR range printed ahead of the EPS range on the same slide; 'Operating Earnings Growth' wording
    ("~10%-12% Total Shareholder Return \u2022 Targeted long-term operating EPS growth of 6%-8% \u2022 Dividend payout ratio of 55%-65%", (6.0, 8.0)),
    ("$72B Capital Plan 2026-2030 ~10% Rate Base CAGR through 2030 7%-9% Annual Operating Earnings Growth Rate With an expected CAGR of greater than 9% through 2030", (7.0, 9.0)),
]

_LT_CONTROLS += [
    # NEE Q4 2023 wording (pass 3 via lt_growth; lt_growth_scan's default passes must NOT match it)
]
def _run_lt_pass3_controls():
    t = "13 \u2022 Expect 6 to 8% annual growth rate through 2026, off the 2024 adjusted EPS expectations range \u2022 From 2021 to 2026"
    r = next(_lt_iter(t, (3,)), None)
    assert r and (r['rate_low_pct'], r['rate_high_pct'], r['base_year'], r['through_year']) == (6.0, 8.0, '2024', '2026'), 'LT PASS3 CONTROL FAILED: %r' % (r,)
    assert lt_growth_scan(t) is None, 'LT PASS3 CONTROL FAILED: pass 3 leaked into the default scan'
    assert next(_lt_iter("Expect 2 to 3% annual growth rate in retail load off the 2024 base", (3,)), None) is None, \
        'LT PASS3 CONTROL FAILED: a load growth rate was accepted'
    # value-preserving preference: same rate on two pages, the later one carries the window
    _pc['__lt_ctl'] = [(11, "Expect annual EPS growth consistently near upper end of 6% to 8% guidance range"),
                       (17, "Expect 6% to 8% EPS CAGR 2026-2030 off 2026 guidance midpoint through 2030")]
    try:
        r = lt_growth('CTL', {'id': '__lt_ctl'})
    finally:
        _pc.pop('__lt_ctl', None)
...
def rate_base(tk, deck):
    ...
    # AEP prints '~10% Rate Base CAGR through 2030' beside a '$72B Capital Plan 2026-2030' label:
    # rate + through-year only, window start taken from the plan window on the same slide.
    thru = r'(?:~|Nearly |nearly |approximately |about )?(?P<r>\d{1,2}(?:\.\d)?)\s*%\s*rate base (?:CAGR|growth)\s*(?:CAGR\s*)?through\s+(?P<y>20\d\d)'
    for rx in pats:
        for pno, body in pages(pid):
            fb = flat(body)
            for m in re.finditer(rx, fb, re.I):
    ...
    for pno, body in pages(pid):
        fb = flat(body)
        for m in re.finditer(thru, fb, re.I):
            r = float(m.group('r')); y = int(m.group('y'))
            if not (2 <= r <= 20): continue
            w = slide_window(fb, m.start(), m.end())
            if w and w.endswith(str(y)):
                a = int(w.split('-')[0])
                return {'rate_pct': r, 'from_year': a, 'to_year': y, 'basis': 'printed', 'page': pno,
                        'note': 'window start %d read from the plan window printed on the same slide' % a,
                        'text': fb[max(0, m.start() - 110): m.end() + 110]}
            return {'rate_pct': r, 'from_year': y - 4, 'to_year': y, 'basis': 'derived', 'page': pno,
                    'note': 'slide prints the rate and the through-year only; five-year window assumed (%d-%d)' % (y - 4, y),
                    'text': fb[max(0, m.start() - 110): m.end() + 110]}
    return None
...
# AEP FINANCING-PLAN TABLE (2026-09-24). AEP prints a full sources-and-uses table in almost
# every deck, in three layouts: annual columns (2021-23; Q4 2023 in REVERSE year order),
# a single total column with each value printed BEFORE its label (Q3/Q4 2024), and
# '2026E | 2027E | 2028E-2030E | Total' (Q3 2025 on). The prose equity/debt extractors
# never fire on it, so AEP's stated equity was a stale $2.3B forward and debt_b was null.
# Row sums are order-invariant, and the table closes on an identity:
#     required capital = net new debt + equity issuances + equity-unit conversions
# where net new debt = gross debt-capital-market issuance - maturities - securitization
# amortizations - short-term repayments. A table whose parse does not close within 2% is
# REFUSED (returns None) - that check is what makes a three-layout parser safe.
_AEP_NUM = r'(?:\$\s*)?(?:\(\s*[\d,]+\s*\)|[\d,]{1,7}(?![\d,])|-(?=\s))'
_AEP_RUN = re.compile(r'\s*((?:' + _AEP_NUM + r'\s*){1,7})')
_AEP_ROWS = [  # key, label regex (first match wins per key)
    ('required', r'Required Capital(?: Market Needs)?(?! Financing)'),
    ('maturities', r'(?:Long-term )?Debt Maturities(?: \(Senior Notes, PCRBs\))?'),
    ('securitization', r'Securitization Amortizations?'),
    ('st_repay', r'Short-term Debt Repayments'),
    ('units', r'Equity Units Conversion'),
    ('dcm_gross', r'Debt Capital Market Activity\d?|Total Debt Capital Market Needs'),
    ('dcm_new', r'Debt Capital Markets? Needs \(New\)\d?'),
]
_AEP_EQ = r'(?:Plus: )?(?:Equity Issuances[^\d$(\-]{0,40}|Asset Monetization / Equity\d?)'

def _aep_vals(s):
    out = []
    for t in re.findall(_AEP_NUM, s):
        t = t.replace('$', '').strip()
        if t == '-': out.append(0.0); continue
        neg = t.startswith('(')
        out.append((-1 if neg else 1) * float(t.strip('() ').replace(',', '')))
    return out

def _aep_row(fb, m, ncols, has_total):
    """Values for one label: the numbers right AFTER it (column layouts), else the one
    number right BEFORE it (the 2024 value-then-label layout). Returns the row total."""
    if ncols == 1:
        before = re.search(r'(' + _AEP_NUM + r')\s*(?:Less: |Plus: )?$', fb[max(0, m.start() - 25):m.start()])
        return _aep_vals(before.group(1))[0] if before else None
    after = _AEP_RUN.match(fb, m.end())
    if after:
        v = _aep_vals(after.group(1))
        if has_total and len(v) >= 4: return v[3]
        if len(v) >= ncols: return sum(v[:ncols])
    before = re.search(r'(' + _AEP_NUM + r')\s*(?:Less: |Plus: )?$', fb[max(0, m.start() - 25):m.start()])
    if before and ncols == 1:
        return _aep_vals(before.group(1))[0]
    return None

def aep_financing(fb):
    if 'Cash from Operations' not in fb or '$ in millions' not in fb.replace('($ in millions)', '$ in millions'):
        return None
    if not re.search(r'Debt Capital Markets? (?:Needs|Activity)', fb): return None
    fb = re.sub(r'\(\$\d+B per year\)', ' ', fb)
    t = re.search(r'(20\d\d)E?\s*[-–]\s*(20\d\d)E?(?: Financing Plan| \$ in millions)', fb)
    yrs = sorted(set(int(y) for y in re.findall(r'\b(20[2-3]\d)E\b', fb)))
    has_total = bool(re.search(r'20\d\dE\s+20\d\dE\s+20\d\dE\s*[-–]\s*20\d\dE\s+Total', fb))
    if has_total or (t and not re.search(r'20\d\dE\s+20\d\dE', fb)):
        ncols, w0, w1 = (3 if has_total else 1), int(t.group(1)), int(t.group(2))
    elif len(yrs) >= 3:
        # column count from the Required Capital row itself - a header can drop its 'E'
        rm = re.search(r'Required Capital', fb)
        run = _AEP_RUN.match(fb, rm.end()) if rm else None
        n = len(_aep_vals(run.group(1))) if run else 0
        if n < 3: return None
        ncols, w0, w1 = n, yrs[0], yrs[0] + n - 1
    else:
        return None
    r = {}
    for k, lab in _AEP_ROWS:
        m = re.search(lab, fb)
        r[k] = _aep_row(fb, m, ncols, has_total) if m else None
    # equity rows: every row in the 'Equity:' block (2025+), else the labelled equity row(s)
    eq, eq_rows = 0.0, []
    blk = re.search(r'Equity:(.*?)Debt Capital Market', fb)
    if blk:
        for m in re.finditer(r'([A-Za-z][A-Za-z0-9 /().–-]*?)\d?\s+((?:' + _AEP_NUM + r'\s*){4})', blk.group(1)):
            v = _aep_vals(m.group(2))
            eq += v[3]; eq_rows.append('%s %s' % (m.group(1).strip(), '{:,.0f}'.format(v[3])))
    else:
        for m in re.finditer(_AEP_EQ, fb):
            v = _aep_row(fb, m, ncols, has_total)
            if v is not None: eq += v; eq_rows.append('%s %s' % (m.group(0).strip(), '{:,.0f}'.format(v)))
    if r['required'] is None or not eq_rows: return None
    req = abs(r['required']); units = r['units'] or 0.0
    matur = abs(r['maturities'] or 0); sec = abs(r['securitization'] or 0); st = abs(r['st_repay'] or 0)
    if r['dcm_gross'] is not None:
        net = abs(r['dcm_gross']) - matur - sec - st
    elif r['dcm_new'] is not None and ncols == 1:        # 2024 layout: '(New)' already excludes maturities
        net = abs(r['dcm_new']) - sec
    elif r['dcm_new'] is not None:                        # 2021-23 layout: '(New)' is gross issuance
        net = abs(r['dcm_new']) - matur - sec - st
    else:
        return None
    close = net + eq + units
    if req <= 0 or abs(close - req) / req > 0.02:
        return None
    return {'equity_b': round(eq / 1000.0, 2), 'debt_b': round(net / 1000.0, 2),
            'window': '%d-%d' % (w0, w1), 'units_b': round(units / 1000.0, 2),
            'required_b': round(req / 1000.0, 2), 'eq_rows': eq_rows,
            'identity': 'required %s = net new debt %s + equity %s + units %s ($M)' % tuple(
                '{:,.0f}'.format(x) for x in (req, net, eq, units))}

# Embedded controls - REAL flattened table texts, one per layout, plus a tampered copy
# (one maturity cell changed) that must be REFUSED by the identity check.
_AEP_FIN_CONTROLS = [
    ("2022-2026 Cash Flows and Financial Metrics $ in millions 2022E 2023E 2024E 2025E 2026E Cash from Operations $ 5,600 $ 6,100 $ 6,400 $ 6,900 $ 7,400 Net Cash Proceeds from Sale of Kentucky Operations1 1,400 - - - - Capital and JV Equity Contributions2 (7,600) (7,100) (8,400) (7,800) (7,400) Other Investing Activities (300) (400) (400) (400) (400) Common Dividends3 (1,600) (1,600) (1,700) (1,700) (1,700) Required Capital $ (2,500) $ (3,000) $ (4,100) $ (3,000) $ (2,100) Financing Required Capital $ (2,500) $ (3,000) $ (4,100) $ (3,000) $ (2,100) Debt Maturities (Senior Notes, PCRBs) (3,000) (2,400) (500) (1,800) (1,400) Securitization Amortizations (100) (200) (200) (100) (100) Equity Units Conversion 805 850 - - - Equity Issuances – Includes DRP - 100 600 700 700 Debt Capital Market Needs (New) $ (4,795) $ (4,650) $ (4,200) $ (4,200) $ (2,900) Financial Metrics", (2.1, 10.95, '2022-2026')),   # Q4 2021 - annual columns
    ("2025E-2029E $ in millions $41,500 Cash from Operations 2,800 Plus: Net Cash Proceeds from Minority Interest Acquisition (54,400) Less: Capital Investments (11,050) Less: Dividends1 (2,300) Less: Other Investing Activities $ (23,450) Required Capital Market Needs (500) Less: Securitization Amortization 2,550 Plus: Equity Issuances – Includes $100M DRP Per Year $ (21,400) Debt Capital Markets Needs (New)2 (11,000) Debt Maturities $ (32,400) Total Debt Capital Market Needs FINANCING PLAN", (2.55, 20.9, '2025-2029')),   # Q4 2024 - value-before-label total column
    ("2026-2030 Financing Plan ($ in millions) 2026E 2027E 2028E-2030E Total Cash from Operations $ 7,800 $ 8,600 $ 30,700 $ 47,100 Capital and JV Equity Contribution (12,900) (14,200) (50,800) (77,900) Other Investing Activities1 (200) (200) (700) (1,100) Common Dividends2 (2,100) (2,200) (6,800) (11,100) Required Capital $ (7,400) $ (8,000) $ (27,600) $ (43,000) Financing Required Capital $ (7,400) $ (8,000) $ (27,600) $ (43,000) Long-term Debt Maturities (2,300) (1,500) (7,200) (11,000) Securitization Amortizations (200) (300) (700) (1,200) Equity: Settlement of 2025 Equity Forward 1,800 - - 1,800 Dividend Reinvestment Plan 180 180 540 900 ATM Program 1,000 - 3,000 ($1B per year) 4,000 Growth Equity - - 3,000 3,000 Debt Capital Market Activity3 $ (6,920) $ (9,620) $ (28,960) $ (45,500) Financial Metrics", (9.7, 33.3, '2026-2030')),    # Q1 2026 - 2026E | 2027E | 2028E-2030E | Total
]
def _run_aep_fin_controls():
    for txt, want in _AEP_FIN_CONTROLS:
        r = aep_financing(txt)
        got = (r['equity_b'], r['debt_b'], r['window']) if r else None
        assert got == want, 'AEP FIN CONTROL FAILED: %r -> %r, want %r' % (txt[:50], got, want)
    bad = _AEP_FIN_CONTROLS[2][0].replace('(11,000)', '(12,000)', 1)
    assert aep_financing(bad) is None, 'AEP FIN CONTROL FAILED: a table that does not close was accepted'

_run_aep_fin_controls()

def _aep_fin_deck(deck):
    for pno, body in pages(deck['id']):
        fb = flat(body)
        if 'Cash from Operations' in fb and 'Debt Capital Market' in fb:
            r = aep_financing(fb)
            if r:
                r['page'] = pno
                return r
...
def equity_plan(tk, deck):
    ...
    if tk == 'AEP':
        r = _aep_fin_deck(deck)
        if r:
            cav = ('[ASSET MONETIZATION / EQUITY COMBINED in one printed row] '
                   if any('Asset Monetization' in x for x in r['eq_rows']) else '')
            units = (' + equity-unit conversions $%gB (not counted as issuance)' % r['units_b']) if r['units_b'] else ''
            return {'equity_b': r['equity_b'], 'basis': 'printed', 'page': r['page'],
                    'text': '%s[%s plan] [AEP financing table, $M] equity rows: %s%s | %s'
                            % (cav, r['window'], '; '.join(r['eq_rows']), units, r['identity'])}
    for pno, body in pages(deck['id']):
...
def debt_plan(tk, deck):
    ...
    if tk == 'AEP':
        r = _aep_fin_deck(deck)
        if r:
            return {'debt_b': r['debt_b'], 'basis': 'printed', 'page': r['page'],
                    'text': '[%s plan] [AEP financing table, $M] NET NEW debt = gross debt-capital-market issuance '
                            'less maturities, securitization amortizations and short-term repayments | %s'
                            % (r['window'], r['identity'])}
    for pno, body in pages(deck['id']):
...
def main():
    ...
    ap.add_argument('--refresh-index', action='store_true',
                    help='rebuild guidance_index.json from data/corpus_manifest.json first (run after build_corpus.py)')
    args = ap.parse_args()
    ...
    if os.path.exists(FLAG) and not args.rebuild:
        flagged = [f for f in json.load(open(FLAG, encoding='utf-8')) if f['ticker'] not in want]
