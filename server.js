...
// Corpus text, held once per document id. Cleared whenever the manifest's mtime moves,
// so re-running build_corpus.py invalidates it without a restart.
const _corpusCache = new Map();
let _corpusManifestMtime = 0;
setInterval(() => {
  try {
    const m = fs.statSync(path.join(DATA_DIR, 'corpus_manifest.json')).mtimeMs;
...
const server = http.createServer(async (req, res) => {
  ...
  // ── Corpus search ────────────────────────────────────────────────────────
  //  GET /api/corpus                      -> the manifest
  //  GET /api/corpus/search?q=&ticker=&type=&limit=
  //
  //  Deliberately NO index format and NO native dependency. The whole ripped
  //  corpus is ~25MB of text, so Node holds it in memory and scans it; a query
  //  costs milliseconds, the result is always current, and there is no second
  //  artefact that can go stale against the .txt files. That is the standing
  //  preference recorded 2026-07-29 - simplest solution, lowest upkeep - applied
  //  to search. If the corpus ever outgrows memory, THEN add an index.
  if (pathname === '/api/corpus') {
    serveFile(req, res, path.join(DATA_DIR, 'corpus_manifest.json'),
              'corpus_manifest.json not found. Run: python scripts\\build_corpus.py');
    return;
  }

  if (pathname === '/api/corpus/search') {
    try {
      const q = url.parse(req.url, true).query;
      const term = String(q.q || '').trim();
      if (term.length < 2) { sendJSON(req, res, 200, { hits: [], note: 'query too short' }); return; }

      const manPath = path.join(DATA_DIR, 'corpus_manifest.json');
      if (!fs.existsSync(manPath)) {
        sendJSON(req, res, 200, { hits: [], error: 'No corpus_manifest.json. Run scripts\\build_corpus.py.' });
        return;
      }
      const man = JSON.parse(fs.readFileSync(manPath, 'utf8'));
      let docs = man.documents || [];
      if (q.ticker) { const t = String(q.ticker).toUpperCase(); docs = docs.filter(d => (d.tickers||[]).includes(t)); }
      if (q.type)   { const ts = String(q.type).split(','); docs = docs.filter(d => ts.includes(d.doc_type)); }

      // phrase match, case-insensitive. Quoted -> exact phrase; otherwise all words.
      const phrase = /^".*"$/.test(term);
      const needles = phrase ? [term.slice(1, -1).toLowerCase()]
                             : term.toLowerCase().split(/\s+/).filter(w => w.length > 1);
      const limit = Math.min(parseInt(q.limit) || 60, 300);
      const hits = [];

      for (const d of docs) {
        if (d.text_layer === false) continue;            // nothing to search; UI says why
        const tp = path.join(DATA_DIR, (d.text_file || '').replace(/\//g, path.sep));
        let text = _corpusCache.get(d.id);
        if (text === undefined) {
          try { text = fs.readFileSync(tp, 'utf8'); } catch (e) { text = null; }
          _corpusCache.set(d.id, text);
        }
        if (!text) continue;
        const low = text.toLowerCase();
        if (!needles.every(w => low.includes(w))) continue;

        // Page attribution walks the [[PAGE n]] markers that rip.py and
        // build_corpus.py both emit, so page numbers mean the same thing in both.
        //
        // Anchor on the RAREST needle, not the first. On "rate base", anchoring on
        // "rate" lands on any page that says "rate" and the snippet reads as noise;
        // anchoring on whichever term occurs least often puts the reader where the
        // uncommon word is. Then require the other terms within a window, so a
        // multi-word query returns passages rather than coincidences.
        const PROX = 400;
        // Try the WHOLE query as a phrase first. "rate base" means the term of art,
        // not a page where "rate" and "base" happen to co-occur - and utility
        // boilerplate co-occurs constantly, so proximity alone returns safe-harbour
        // paragraphs. Fall back to rarest-term + proximity only when the phrase is
        // genuinely absent, so nothing is dropped.
        const wholePhrase = needles.length > 1 ? term.toLowerCase() : null;
        const usePhrase = wholePhrase && low.includes(wholePhrase);
        const freq = needles.map(w => low.split(w).length - 1);
        const anchor = usePhrase ? wholePhrase : needles[freq.indexOf(Math.min(...freq))];
        const others = usePhrase ? [] : needles.filter(w => w !== anchor);
        const near = (at) => others.every(w => {
          const lo = Math.max(0, at - PROX), hi = Math.min(low.length, at + PROX + anchor.length);
          return low.lastIndexOf(w, hi) >= lo;
        });

        const mk = (at) => {
          const before = text.lastIndexOf('[[PAGE ', at);
          const pnum = before >= 0 ? parseInt(text.slice(before + 7, text.indexOf(']]', before))) : null;
          const s0 = Math.max(0, at - 150), s1 = Math.min(text.length, at + 230);
          return { page: pnum,
                   snippet: text.slice(s0, s1).replace(/\[\[PAGE \d+\]\]/g, ' ').replace(/\s+/g, ' ').trim() };
        };

        const strong = [], weak = [];
        let idx = 0;
        while (strong.length + weak.length < 40) {
          const at = low.indexOf(anchor, idx);
          if (at < 0) break;
          (near(at) ? strong : weak).push(at);
          idx = at + anchor.length;
        }
        // Prefer passages where every term co-occurs; fall back to bare anchor hits
        // so a document is never dropped just because the terms sit far apart.
        const chosenAll = (strong.length ? strong : weak);
        // Group by page and rank pages by how many times the term appears ON that page.
        // Taking the first N occurrences instead surfaces p.1-2 of every deck, because
        // the safe-harbour paragraph mentions everything once. The page that repeats
        // the term is the page actually about it.
        const byPage = new Map();
        for (const at of chosenAll) {
          const h = mk(at);
          const k = h.page == null ? -1 : h.page;
          const e = byPage.get(k);
          if (e) e.n++; else byPage.set(k, { ...h, n: 1 });
        }
        const pages = [...byPage.values()]
          .sort((a, b) => (b.n - a.n) || ((a.page ?? 1e9) - (b.page ?? 1e9)))
          .slice(0, 5);
        const total = chosenAll.length;
        hits.push({
          id: d.id, title: d.title, tickers: d.tickers, doc_type: d.doc_type,
          event: d.event, date: d.date, period: d.period, pages: d.pages,
          url: d.url, matches: total, strong: strong.length,
          match_mode: needles.length === 1 ? 'term'
                    : usePhrase ? 'phrase'
                    : (strong.length ? 'proximity' : 'terms-apart'),
          page_hits: pages,
        });
      }

      // Rank by co-occurrence count, then recency. A 2026 call beats a 2021 deck at
      // equal strength, which is what you want when the question is "what are they
      // saying about X" rather than "when did they first mention X".
      hits.sort((a, b) => (b.strong - a.strong) || (b.matches - a.matches) ||
                          String(b.date || '').localeCompare(String(a.date || '')));
      sendJSON(req, res, 200, {
        query: term, scanned: docs.length, hit_docs: hits.length,
        skipped_no_text: docs.filter(d => d.text_layer === false).length,
        hits: hits.slice(0, limit),
      });
    } catch (e) {
      log(`corpus search error: ${e.message}`);
      sendJSON(req, res, 200, { hits: [], error: e.message });
    }
    return;
  }
