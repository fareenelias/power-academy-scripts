...
def extract_rate_base_from_text(text: str, opco_labels: list,
    ...
    Strategies: total_rate_base_row > regulatory_overview > dollar_phrase > cagr_context > balance_sheet
    """
    results = []

    # Strategy 1: "Total Rate Base $X.X $X.X ..." table row
    p1 = re.compile(
        r'(?:\w[\w\s&]*?\s+)?Total\s+Rate\s+Base\s+((?:\$?\s*[\d,\.]+\s+){1,8})',
        re.IGNORECASE
    )
    for m in p1.finditer(text):
        vals = [float(v.replace(",","")) for v in re.findall(r"[\d,\.]+", m.group(1))
                if 0.2 < float(v.replace(",","")) < 800]
        if not vals:
            continue
    ...
    # Strategy 2: Per-opco "Year-End Rate Base ($B) $X.X" slides
    p2 = re.compile(
        r'(?:20\d\d\s+)?(?:Year-End\s+)?Rate\s+Base\s+\(\$B\)\s+\$?\s*([\d,\.]+)',
        re.IGNORECASE
    )
    opco_matches = []
    for m in p2.finditer(text):
        ...
        opco_matches.append({"opco": opco, "value_b": val, "year": year})
    if opco_matches:
        total = round(sum(v["value_b"] for v in opco_matches), 1)
        years = [v["year"] for v in opco_matches if v["year"]]
        results.append({
            "total_b": total, "year": years[0] if years else None,
            "future_values": {}, "opco_breakdown": opco_matches,
            "strategy": "regulatory_overview_slides",
            "context": f"Sum of {len(opco_matches)} opco values",
        })

    # Strategy 3: "$X.XB rate base" phrase
    for pat in [
        re.compile(r'(?:total\s+|projected\s+|year.end\s+)?rate\s+base\s+'
                   r'(?:of\s+)?(?:~\s*)?\$\s*([\d,\.]+)\s*(?:billion|B\b|bn\b)',
                   re.IGNORECASE),
        re.compile(r'(?:~\s*)?\$\s*([\d,\.]+)\s*(?:billion|B\b|bn\b)\s+'
    ...
    # Strategy 4: Net utility plant from balance sheet (Tier 3 fallback)
    bs_patterns = [
        re.compile(r'[Rr]egulated\s+utility\s+plant,?\s+net\s+\$?\s*([\d,]+)', re.IGNORECASE),
        re.compile(r'[Nn]et\s+utility\s+plant\s+\$?\s*([\d,]+)', re.IGNORECASE),
        re.compile(r'[Nn]et\s+[Pp]roperty,?\s+[Pp]lant\s+&?\s+[Ee]quipment\s+\$?\s*([\d,]+)', re.IGNORECASE),
    ]
    for pat in bs_patterns:
        for m in pat.finditer(text):
            # Balance sheet values are in millions
            ...
            results.append({
                ...
                "context": f"Net utility plant (GAAP) from press release balance sheet",
                "is_tier3": True,
            })
            break  # one match is enough

    # Deduplicate and rank
    RANK = {"total_rate_base_row": 1, "regulatory_overview_slides": 2,
            "dollar_rate_base_phrase": 3, "cagr_context": 4,
