...
# --- Cover-based resolution (preferred over the filename) --------------------
# S&P transcript covers print the exchange tag and fiscal quarter explicitly:
#     "PG&E Corporation NYSE:PCG"
#     "FQ2 2026 Earnings Call Transcripts"
# S&P's own FILENAMES carry neither a ticker nor a quarter -- e.g.
# "PGE Corporation_Earnings Call_2026-07-23_English.pdf" drops the ampersand, so
# the "pg&e" name-map key misses and the file rips as UNK; and with no quarter in
# the name, period_from_name falls through to the bare-year FY branch. Reading the
...
def ticker_from_cover(cover):
    """Exchange tag off the cover page. Accepted ONLY if it is a coverage ticker --
    an unrecognised tag falls through to the filename rather than inventing a name.
    This also blocks the PGE/POR trap: 'PGE' is Portland General's real ticker, so a
    looser matcher could file a PG&E call under POR. PGE is not in the set."""
    for m in EXCHANGE_TICKER_RE.finditer(cover or ''):
        t = m.group(1).upper()
        if t in COVERAGE_TICKERS:
            return t
    return None

def period_from_cover(cover):
    """Fiscal quarter off the cover ('FQ2 2026' -> 'Q2 2026'). Returns None when no
    explicit quarter is present -- never guesses FY, since the bare-year fallback is
    exactly the wrong answer that filename parsing already produces."""
    m = re.search(r'\bF?Q([1-4])[ ,_-]*(20\d{2})', cover or '', re.I)
...
def main():
    ...
    for pdf in iter_pdfs(a.paths, a.recursive):
        ...
        try:
            doc=fitz.open(pdf); dtype=sniff_doc_type(doc)
            _cover_probe = doc[0].get_text() if doc.page_count else ""
            doc.close()
        ...
        if dtype=="transcript":
            ...
            # MUST mirror the write path exactly. If the probe resolved names the old
            # filename-only way it would look for UNK_FY-2026.txt, find the stale file,
            # and skip the PDF forever.
            _tk = ticker_from_cover(_cover_probe) or ticker_from_name(fn)
            _pd = period_from_cover(_cover_probe) or period_from_name(fn)
            b=f"{_tk}_{_pd.replace(' ','-')}"
