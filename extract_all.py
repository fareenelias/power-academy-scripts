...
# Single consolidated text output dir. rip.py DEFAULTS to a per-folder
# <Quarter Folder>\text\, which fragments output and hides it from the health
# check below (which reads only this root path) — so --out is always passed.
TRANSCRIPTS_TEXT = r'E:\PowerAcademy\Documents\transcripts\text'
# rip.py replaced rip_earnings.py on 2026-07-18. This constant was never
# repointed, so the retired ripper kept running: it resolves ticker/period from
# the FILENAME, and S&P filenames carry neither, which is what produced
# UNK_FY-2026.txt. rip.py reads the cover page instead.
RIP_SCRIPT      = r'E:\PowerAcademy\scripts\rip.py'
...
# AQN DOES hold earnings calls and is deliberately NOT excluded here. It joined the
# universe on 2026-07-31 with no transcripts on file, so it will show as a genuine
# coverage gap until one is ripped -- which is the correct signal, not a bug.


def parse_ticker_filter():
    """
    --tickers NEE,PCG   (also accepts --tickers=NEE,PCG, repeatable)

    Restricts which workbooks are READ. Safe because both outputs are
    merge-preserving: unprocessed companies are carried through from disk
    untouched. Without this flag every workbook in REPORTS_DIR is re-read,
    so a month-old file overwrites its company's fields on every run.
    """
    wanted = set()
    for i, a in enumerate(sys.argv):
        if a == '--tickers' and i + 1 < len(sys.argv):
            wanted.update(t.strip().upper() for t in sys.argv[i + 1].split(',') if t.strip())
        elif a.startswith('--tickers='):
            wanted.update(t.strip().upper() for t in a.split('=', 1)[1].split(',') if t.strip())
    return wanted


def workbook_mtime_iso(path):
    """Filesystem mtime of a workbook, second-resolution ISO. Compared against the
    per-company source_mtime stamp to decide whether a workbook needs re-reading."""
    try:
...
def rip_transcripts():
    ...
    try:
        # -r is MANDATORY: the PDFs live in per-quarter subfolders and rip.py is
        # NOT recursive by default, so a bare folder arg rips ZERO files silently.
        # --out keeps every .txt and the single _earnings_index.json in one place,
        # which is where report_call_notes() looks.
        subprocess.run([sys.executable, RIP_SCRIPT, TRANSCRIPTS_DIR,
                        '-r', '--out', TRANSCRIPTS_TEXT], check=False)
...
def report_call_notes():
    ...
    idx_path = os.path.join(TRANSCRIPTS_TEXT, '_earnings_index.json')
...
def report_rra():
    ...
    try:
        ...
        return
    # rra_states.json (schema v1.2) nests profiles under a top-level "states" key,
    # alongside sibling keys state_names / company_state_map. Iterating the top
    # level counted THOSE as states — hence a permanent "3 state profiles" reading
    # regardless of the real count, and a placeholder scan that never inspected an
    # actual profile. Descend into "states"; keep a flat fallback for legacy files.
    if isinstance(rra.get('states'), dict):
        states = rra['states']
    else:
        states = {k: v for k, v in rra.items()
                  if not k.startswith('_') and k not in ('state_names', 'company_state_map')}

    remaining = rra.get('_states_remaining') or []
    placeholder_states = []
    ...
    print(f'  [RRA] {len(states)} state profiles'
          + (f'  ({len(placeholder_states)} with pending/placeholder categories: '
             f'{", ".join(sorted(placeholder_states))})'
             if placeholder_states else '  (all categories populated)')
          + (f'  [{len(remaining)} not yet built: {", ".join(sorted(remaining))}]'
             if remaining else ''))
...
def main():
    ...
    only = parse_ticker_filter()
    if only:
        unknown = only - set(COVERAGE_UNIVERSE)
        if unknown:
            print(f'  [WARN] not in the 23-name coverage universe: {", ".join(sorted(unknown))}')
        kept  = [f for f in files if (ticker_from_filename(f) or '').upper() in only]
        found = {(ticker_from_filename(f) or '').upper() for f in kept}
        missing = only - found
        if missing:
            print(f'  [WARN] no workbook found in {REPORTS_DIR} for: {", ".join(sorted(missing))}')
        files = kept
        print(f'\n--tickers {",".join(sorted(only))} — '
              f'{len(files)} of {len(only)} requested workbook(s) matched')

    # ── Incremental: skip workbooks whose mtime matches the last extraction ───
    # Each company stores source_workbook + source_mtime. A workbook is re-read
    # only when its filename or mtime differs from what produced the current data.
    # --force re-reads everything: ALWAYS use it after changing extraction logic,
    # since unchanged workbooks would otherwise never pick up the new code.
    force = ('--force' in sys.argv) or ('--all' in sys.argv)
    unchanged = []
    if not force:
        fresh = []
        for f in files:
            t  = (ticker_from_filename(f) or '').upper()
            co = companies.get(t) or {}
            if (co.get('source_workbook') == os.path.basename(f)
                    and co.get('source_mtime') == workbook_mtime_iso(f)):
                unchanged.append(t)
            else:
                fresh.append(f)
        files = fresh
    ...
    if unchanged:
        print(f'  Unchanged since last extract ({len(unchanged)}): '
              f'{", ".join(sorted(unchanged))}')
    if force:
        print('  --force: re-reading every workbook')

    print(f'\nProcessing {len(files)} Excel files from {REPORTS_DIR}\n')

    if not files:
        print('  Nothing new to extract — no workbook has changed.')
        print('  (Use --force to re-read all, or --tickers X to target one.)\n')
    ...
    for fpath in files:
        ...
        # 'extracted' used to be stamped ONLY when a ticker was first created, so
        # every company carried its original creation date forever and there was
        # no way to read data vintage per name. Stamp it on every run, alongside
        # the workbook it came from.
        if ticker not in companies:
            companies[ticker] = {'ticker': ticker}
        companies[ticker]['extracted']       = TODAY
        companies[ticker]['source_workbook'] = os.path.basename(fpath)
        companies[ticker]['source_mtime']    = workbook_mtime_iso(fpath)
    ...
    # executives_export.json is MERGE-preserving.
    # It used to be a full rebuild (execs_output starts empty and was dumped
    # straight over the file), so a PARTIAL run — e.g. dropping only 2 workbooks
    # into data\reports\ — silently wiped the other 21 companies. Now only the
    # tickers actually processed this run are replaced; everything else on disk
    # is preserved. Use --rebuild-execs to force the old full-overwrite behaviour
    # (e.g. to drop a ticker that has left the coverage universe).
    rebuild_execs = '--rebuild-execs' in sys.argv
    execs_on_disk = {}
    if os.path.exists(EXECS_JSON) and not rebuild_execs:
        try:
            with open(EXECS_JSON, 'r', encoding='utf-8') as f:
                execs_on_disk = json.load(f)
        except (json.JSONDecodeError, OSError, UnicodeDecodeError) as e:
            print(f'  [WARN] Could not read existing {EXECS_JSON} ({e})')
            print(f'         Writing THIS RUN ONLY — {len(execs_output)} companies. '
                  f'Restore from backup if that is not what you want.')
            execs_on_disk = {}

    preserved = sorted(t for t in execs_on_disk if t not in execs_output)
    execs_merged = {**execs_on_disk, **execs_output}
    execs_merged = {t: execs_merged[t] for t in sorted(execs_merged)}

    with open(EXECS_JSON, 'w', encoding='utf-8') as f:
        json.dump(execs_merged, f, indent=2, ensure_ascii=False)

    if rebuild_execs:
        print(f'  executives_export.json: FULL REBUILD — {len(execs_output)} companies')
    elif preserved:
        print(f'  executives_export.json: {len(execs_output)} updated '
              f'({", ".join(sorted(execs_output))}), {len(preserved)} preserved')
